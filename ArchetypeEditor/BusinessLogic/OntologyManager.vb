...
Public Class OntologyManager
    ...
    Private Sub InitialiseOntologyManager(Optional ByVal LanguageCode As String = "")
    ...
    Public Sub UpdateTerm(ByVal aterm As RmTerm)
        ...
        Else
            If InStr(aterm.Description, "@ internal @") = 0 Then
        ...
        If (mLanguageCode = Me.PrimaryLanguageCode) Then
            If mOntology.IsMultiLanguage Then
                If mReplaceTranslations = 0 Then
                    If MessageBox.Show(AE_Constants.Instance.ReplaceTranslations, AE_Constants.Instance.MessageBoxCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
    ...
    Private Sub InitialiseTables()
        ...
        new_relation = New DataRelation("TerminologiesConstraintBindings", mTerminologiesTable.Columns(0), mConstraintBindingsTable.Columns(0))
        mLanguageDS.Relations.Add(new_relation)
    ...
    'Public Sub ConvertToADL(ByVal aNewOntology As ADL_Ontology)

    '    ' set the primary language codes and available languages
    '    aNewOntology.SetPrimaryLanguage(mOntology.PrimaryLanguageCode)
    '    For Each languageRow As DataRow In Me.mLanguagesTable.Rows
    '        aNewOntology.AddLanguage(CStr(languageRow(0)))
    '    Next

    '    ' set the term definitions
    '    For Each termDefinitionRow As DataRow In Me.mTermDefinitionsTable.Rows
    '        Dim newTerm As New Term(CStr(termDefinitionRow(1)))
    '        newTerm.Text = CStr(termDefinitionRow(2))
    '        newTerm.Description = CStr(termDefinitionRow(3))
    '        newTerm.Language = CStr(termDefinitionRow(0))
    '        aNewOntology.AddTerm(newTerm)
    '    Next

    '    ' set the constraint definitions
    '    For Each constraintDefinitionRow As DataRow In Me.mConstraintDefinitionsTable.Rows
    '        Dim newTerm As New Term(CStr(constraintDefinitionRow(1)))
    '        newTerm.Text = CStr(constraintDefinitionRow(2))
    '        newTerm.Description = CStr(constraintDefinitionRow(3))
    '        newTerm.Language = CStr(constraintDefinitionRow(0))
    '        aNewOntology.AddTerm(newTerm)
    '    Next

    '    ' set the terminologies
    '    For Each terminologyRow As DataRow In mTerminologiesTable.Rows
    '        aNewOntology.AddTerminology(CStr(terminologyRow(0)))
    '    Next

    '    ' todo: add the terminology bindings
    '    For Each termBindingRow As DataRow In Me.mTermBindingsTable.Rows

    '    Next

    '    ' todo: add the constraint bindings
    '    For Each constraintBindingRow As DataRow In Me.mConstraintBindingsTable.Rows

    '    Next

    'End Sub

    'Sub New(ByVal Default_Language_Code As String)
    Sub New()
